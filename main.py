...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, ent_embed_dim, n_depth, l2_weight, lr, optimizer_type,
    ...
    config.exp_name = f'RGDA_{dataset}_neigh_{neighbor_sample_size}_embed_{ent_embed_dim}_depth_' \
    ...
    if r == 1 and kfold == 1:
        ...
        np.savetxt(LOG_DIR+f'/RGDA_model_{dataset}_{aggregator_type}_case{c}.txt',np.array(prediction),fmt="%s")
        #
        ...
        if c == 3:
            ...
            np.savetxt(LOG_DIR+f'/RGDA_model_{dataset}_{aggregator_type}_{c}_neiborone.txt',neibor_entity_one,fmt="%d")
            np.savetxt(LOG_DIR+f'/RGDA_model_{dataset}_{aggregator_type}_{c}_neibortwo.txt',neibor_entity_two,fmt="%d")
            np.savetxt(LOG_DIR+f'/RGDA_model_{dataset}_{aggregator_type}_{c}_at_one.txt',attention[0],fmt="%.3f")
            np.savetxt(LOG_DIR+f'/RGDA_model_{dataset}_{aggregator_type}_{c}_at_two.txt',attention[1],fmt="%.3f")
