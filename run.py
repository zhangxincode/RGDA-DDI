...
def process_data(K:int,dataset:str,N:int,HOP:int,B:int,LR:float,ND:int,LC:int,C:int,R:int,S:int,MulHead:int):
    ...
    if dataset == 'kegg':
        ...
        train_examples = read_example_file(os.path.join(RAW_DATA_DIR, dataset, 'raw_data/drugbank/train.txt'), SEPARATOR[dataset], drug_vocab)
        ...
        test_examples = read_example_file(os.path.join(RAW_DATA_DIR, dataset, 'raw_data/drugbank/test.txt'), SEPARATOR[dataset], drug_vocab)
        ...
        dev_examples = read_example_file(os.path.join(RAW_DATA_DIR, dataset, 'raw_data/drugbank/dev.txt'), SEPARATOR[dataset], drug_vocab)
...
def cross_validation(K_fold,total_examples,dataset,neighbor_sample_size,HOP,B,LR,ND,LC,C,R,S,MulHead):
    ...
    for t in aggregator_types:
        ...
        for i in reversed(range(0,K_fold)):
            ...
            n_depth=HOP,
            l2_weight=1e-7,
            lr=config.lr,#1e-2/5e-3
            optimizer_type='adam',
            batch_size=config.batch_size,#1024
            aggregator_type=t,
            n_epoch=config.n_epoch,#50
...
if __name__ == '__main__':
    ...
    # parser.add_argument("-c","--C",help="this parameter decide which cross-attention layer to be used, the default is 3",type=int,default="2")
    ...
    # 5 kegg 16 1 1024 0.01 64 1 2 0 0 1
    config = ModelConfig()
    process_data(config.K_Fold,config.dataset,config.neighbor_sample_size,config.n_depth,config.batch_size,config.lr,config.ent_embed_dim,config.lc,config.c,config.r,config.s, config.head)
